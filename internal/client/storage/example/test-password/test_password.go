...
func main() {
	...
	sm, err := storage.InitializeStorage(dbPath, username, "my-super-secret-password", nil, nil)
	if err != nil {
		fmt.Printf("❌ Ошибка: %v\n", err)
		return
	}
	...
	sm2, err := storage.InitializeStorage(dbPath, username, "wrong-password-123", nil, nil)
	if err != nil {
